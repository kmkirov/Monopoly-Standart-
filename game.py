...
class Game():
    ...
    def roll_dice(self):
        ...
        # dali imame chift
        currnet_possition = self.players[
            self.current_player].move_from_to(0)[0]
        ...
        if self.rolls_counter == 2 and a == b:  # 2
            ...
            print('endturn case 2',
                  [steps, END_TURN, self.current_player_index()])
            ...
            if a == b:
                ...
                print(
                    'endturn case 4'[a + a, position[0], END_TURN], self.current_player_index())
                self.rolls_counter = 3
                ...
                if self.players[self.current_player].jail() == MUST_PAY_JAIL:
                    ...
                    self.players[self.current_player].change_jail(
                        FREE_FROM_JAIL)
                    self.move_player(a + b)
                    print(
                        'endturn case 5', [a + b, END_TURN], self.current_player_index())
                    self.rolls_counter = 3
                    return [a + b, END_TURN]
            ...
            # auto jail added :D
            print(
                'endturn case 6'[steps, END_TURN], self.current_player_index())
            self.rolls_counter = 3
            ...
            if position[1] < position[0]:
                #minava go
                self.players[self.current_player_index()].add_money(200)
            print('endturn case 7:', self.rolls_counter,
                  a == b, self.current_player_index())
    ...
    def move_player_by_rolled(self, steps):
        ...
        if not self.players[self.current_player].jail():
            #return the positon
            return self.players[self.current_player].move_from_to(steps)
        else:
            return JAIL
    ...
    def take_fee(self):
        ...
        renting_result = self.mapa[building_index].take_fee(
    ...
    def stay_in_jail(self):
        self.players[self.current_player].change_jail(
            self.players[self.current_player].jail() + 1)
    ...
    def jail_decision(self, money, steps=0):
        ...
        if money == JAIL_FEE:  # pay for freedom
            ...
            self.move_player(steps)
        #must pay ont 3rd roll
        elif money == 0 and self.playes[self.current_player].jail() == MUST_PAY_JAIL:
            self.playes[self.current_player].pay_money(JAIL_FEE)
            self.playes[self.current_player].change_jail(FREE_FROM_JAIL)
            self.move_player(steps)
        else:
            self.playes[self.current_player].change_jail(
                self.playes[self.current_player].jail() + 1)  # +1 for being there
    ...
    def community_chest(self, player):
        ...
        """
        card_chest = self._COMMUNITY_CHEST[
            self._comunity_chest_index]  # izbiram karta
    ...
    def Chance(self, player):  # ne e testvan
        ...
        second_len = len(card_chest[1])  # bezpolezen ..:)
        player_pos = player.move_from_to(0)[
            0]  # self.player_position(player_name)
