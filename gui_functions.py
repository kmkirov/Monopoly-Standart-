...
players_mortage_icons[3] = pygame.image.load(
    'players_icons/ship - mortage.gif')
...
def register_player(game, display):  # testvan!!!!
    ...
    textImg = font.render(
        "   Register player (type and pres enter)", 1, (255, 0, 0))
...
def trader(game, player_name, window):  # raboti
    ...
    # vyvejdaneto
    current_offer_buildings = ask(window, 'current player buildings')
    # vyvejdaneto
    current_offer_money = int(ask(window, 'current player money offer'))
    # vyvejdaneto
    other_offer_buildings = ask(window, 'other player buildings')
    other_offer_money = int(ask(window, 'other player money'))  # vyvejdaneto
    current_offer_buildings = [int(i)
                               for i in current_offer_buildings.split()]
    ...
    game.trade_buildings(player_name, current_offer_buildings,
                         current_offer_money, other_player, other_offer_buildings, other_offer_money)
...
def mourtager(game, window):  # testvana da sloja animaciq za poziciq
    ...
    if game.mourtage(int(building_number)):
        ...
        buy_or_mortage_property(
            game.current_player_index(), int(building_number), MOURTAGE, window)
    # mourtage animation
#
...
def unmourtager(game, window):  # testvana da sloja animaciq za poziciq
    ...
    font = pygame.font.Font(None, 26)
    textImg = font.render(
        "  unmourtage write just one building", 1, (255, 0, 0))
    window.blit(textImg, center)  # za da e nai otgore
    ...
    if game.unmourtage(int(building_number)):
        ...
        buy_or_mortage_property(
            game.current_player_index(), int(building_number), BUY, window)
    # unmortage animation
    ...
    for event in pygame.event.get():  # event handling loop
        if event.type == QUIT or (event.type == KEYUP and event.key == K_ESCAPE):
            ...
            if Free.pressed([mousex, mousey]):
                # globqva s 50 i mest i mesti na novo mqsto kato se ochakva za
                # mqsotto da se proveri kakvo e
...
def auctioner(game, building_number, pp):
    ...
    textImg1 = font.render(
        "   building name is " + a + 'price is ' + a, 1, (255, 0, 0))
...
def winner(pp, game):
    font = pygame.font.Font(None, 30)
    textImg = font.render(
        " winner is " + str(game.render_name_and_budget()[0]), 1, (255, 0, 0))
    #textImg1 = font.render( "   building name is " + game.at(building_position)[1], 1, (255,0,0))
    catImg1 = pygame.image.load('pictures/bG.jpg')
    pp.blit(catImg1, blue_bg)
    pp.blit(textImg, center)
...
#action_type in [BUY,MOURTAGE]
def buy_or_mortage_property(player_index, new_pos, action_type, display):
